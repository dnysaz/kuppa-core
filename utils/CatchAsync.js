/**
 * Kuppa Engine - Async Wrapper (Optimized & Safe)
 * Mendeteksi Middleware (req, res, next) vs Controller (process)
 * Versi Full Fix - Aman untuk Middleware dan Controller
 */
...
const catchAsync = (fn) => {
    return (req, res, next) => {
        ...
        // 1. LOGIKA DETEKSI:
        // Jika fungsi yang dibungkus adalah Middleware (punya 3 argumen: req, res, next),
        // maka kita berikan akses langsung tanpa mengubah apapun.
        if (fn.length === 3) {
            return Promise.resolve(fn(req, res, next)).catch(next);
        }

        // 2. LOGIKA CONTROLLER:
        // Jika fungsi punya 1 argumen, kita anggap itu adalah Controller Kuppa,
        // jadi kita buatkan objek 'process' agar helper Laravel-mu tetap jalan.
        const process = {
            req, res, next, 
            http: axios, 
            body: req.body,
